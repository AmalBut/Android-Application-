package com.example.advancepizza.ui.profile;
...
import com.example.advancepizza.Hash;
import com.example.advancepizza.HomeActivity;
import com.example.advancepizza.HomeAdminActivity;
...
public class ProfileFragment extends Fragment {
    ...
    public void onResume() {
        ...
        button_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String gender = currentUser.getString(5);
                String permission = currentUser.getString(6);
                String password = editText_password.getText().toString();
                String confirmPassword = editText_confirmPassword.getText().toString();
                String email = currentUser.getString(0);

                User user = new User(firstName,lastName,gender, email, password, phoneNumber,null,permission);
                        ...
                        .setPositiveButton("save", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int id) {
                                ...
                                if (changePass){
                                    ...
                                    dataBaseHelper.updateUserPassword(email, Hash.hashPassword(password));
                                ...
                                currentUser=User.currentUser;
                                if (currentUser.getString(6).equals("Customer")){
    ...
    public boolean checkInputFields(User user, String confirmPassword){
        ...
        if (user.getFirstName().replace(" ", "").isEmpty() || user.getFirstName().length() < 3){
            editText_firstName.setError("Please enter your first name and it must be at least 3 characters long");
            isValid = false;
        }
        if (user.getLastName().replace(" ", "").isEmpty() || user.getLastName().length() < 3){
            editText_lastName.setError("Please enter your last name and it must be at least 3 characters long");
            isValid = false;
        }
        ...
        } else if(user.getPhoneNumber().length() != 10 || !user.getPhoneNumber().startsWith("05")){
            editText_phoneNumber.setError("Phone number must be exactly 10 digits long and start with 05");
        }
        ...
        if (!user.getPassword().replace(" ","").isEmpty()){
            if (user.getPassword().length() < 8) {
                ...
                btn_visible_password.setVisibility(View.INVISIBLE);
                isValid = false;
            } else if (!user.getPassword().matches("^(?=.*[A-Za-z])(?=.*\\d).+$")) {
                editText_password.setError("Password must be at least 8 characters long and contain at least one letter and one number");
                btn_visible_password.setVisibility(View.INVISIBLE);
                isValid = false;
            }
    ...
    public void assignInfoToInputFields(Cursor currentUser){
        textView_email.setText(currentUser.getString(0));
        editText_firstName.setText(currentUser.getString(1));
        editText_lastName.setText(currentUser.getString(2));
        editText_phoneNumber.setText(currentUser.getString(4));

        if (currentUser.getString(5).equals("Male")){
